...
<body onbeforeunload="pageAnalytics()" id="body-home">
	...
	<div id="loading-anim">
		...
		<lottie-player src="https://assets7.lottiefiles.com/packages/lf20_pHl01w.json"  background="transparent"  speed="1"  style="width: 300px; height: 300px;"  loop  autoplay></lottie-player>
	</div>


	<?php
	$myip = get_client_ip();
	$select2 = mysqli_query($conn,"SELECT * FROM recommend WHERE ip='$myip'  ORDER BY id DESC");
	...
	<div class="review-slider">
		...
		<div class="review-con-holder">
			...
			while($s2 = mysqli_fetch_array($select2)){
				...
				$select3 = mysqli_query($conn,"SELECT * FROM head_earphone WHERE by_info='$Bname' LIMIT 5");
				...
				$select3 = mysqli_query($conn,"SELECT * FROM mobiledataset WHERE by_info='$Bname' LIMIT 5");
				while($s3 = mysqli_fetch_array($select3)){
					$pid = $s3['id'];
					$pname = $s3['Product_name'];
					$pimg = $s3['Product_img'];
				...
				$select3 = mysqli_query($conn,"SELECT * FROM laptopdataset WHERE by_info='$Bname' LIMIT 5");
				while($s3 = mysqli_fetch_array($select3)){
					$pid = $s3['id'];
					$pname = $s3['Product_name'];
					$pimg = $s3['Product_img'];
