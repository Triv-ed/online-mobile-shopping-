  ...
  <div class="w3-container">
    ...
    <table class="w3-table w3-striped w3-bordered w3-border w3-hoverable w3-white">
      ...
      while($s = mysqli_fetch_array($select)){
        ...
        echo '
        <tr>
          <td><a href="more.php?pid='.$pid.'&cat='.$cat.'" target="_blank">'.$title.'</a></td>
          <td>'.$views.'</td>
          <td>'.$pimp.'</td>
          <td>'.$ctr.'%</td>
          <td>'.$cal_avg.'</td>
      ...
      $select1 = mysqli_query($conn,"SELECT DISTINCT pid,cat FROM product_impression");
      while($s1 = mysqli_fetch_array($select1)){
        $pid1 = $s1['pid'];
        $cat2 = $s1['cat'];
        $puniq = "prod".$pid1.$cat2;
        if(!in_array($puniq, $product_array)){
          //echo $pid1;
          $pimp = productImpressions($conn,$pid,$cat);
          $ctr = round((0/$pimp)*100,2);
          $prod_name = getProductDetails($conn,$pid1,$cat2,"Product_name");
          echo '
          <tr>
            <td>'.$prod_name.'</td>
            <td>0</td>
            ...
            <td>N/A</td>
            <td>N/A</td>
            <td><a href="../php/discription.php?id='.$pid1.'cat='.$cat2.'" target="_blank"><i class="fa fa-external-link"></i></a></td>
          </tr>
          ';
        }
      }
